...
export function AuthProvider({ children }) {
  ...
  // Every action below only touches module-level helpers and the (stable) setAccessToken, so they never
  // need to change — useCallback([]) keeps them, and so the memoised context value, stable across renders.
  const persistTokens = useCallback((newAccessToken, newRefreshToken, remember) => {
  ...
  const verifyTwoFactor = useCallback(
    async (twoFactorToken, code, remember = true) => {
  ...
  const loginWithTokens = useCallback(
    (newAccessToken, newRefreshToken) => persistTokens(newAccessToken, newRefreshToken, true),
    [persistTokens]
  );
  ...
  const value = useMemo(
    ...
    [accessToken, claims, login, verifyTwoFactor, loginWithTokens, register, logout, switchFamily]
  );
